<div class="content-wrapper" style="min-height: 1200.88px;">
              ...
              <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4"><div class="row"><div class="col-sm-12 col-md-6"><div class="dataTables_length" id="example1_length"><label>Show <select time_in="example1_length" aria-controls="example1" class="custom-select custom-select-sm form-control form-control-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label></div></div><div class="col-sm-12 col-md-6"><div id="example1_filter" class="dataTables_filter" style="float: right;"><label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="example1"></label></div></div></div><div class="row"><div class="col-sm-12"><table id="example1" class="table table-bordered table-striped dataTable dtr-inline collapsed" role="grid" aria-describedby="example1_info">
                ...
                <?php
                    foreach($data["list"] as $arr)
                    {
                        ?>
                        <tr>
                            <td style="width:10%" id="id_Schedule"><?= $arr["id_schedule"]?></td>
                            <td style="width:35%" id="time_in"><?= $arr["time_in"]?></td>
                            <td style="width:35%" id="time_out"><?= $arr["time_out"]?></td>
                            <td style="width:30%">
  ...
  <script>
    $("#add").click(function(){
      var time_in = $("#modal-add #time_in").val();
      var time_out = $("#modal-add #time_out").val();
      $.ajax({
        type: "POST",
        url: "./Schedule/add",
        data: {time_in:time_in, time_out:time_out},
        dataType: "Json",
    ...
    $(".edit").click(function(){
      ...
      $("#modal-secondary #time_in").val($(this).parents("tr").find("td:nth-child(2)").text());
      $("#modal-secondary #time_out").val($(this).parents("tr").find("td:nth-child(3)").text());
    })
    $("#edit").click(function(){
      var id = $("#modal-secondary #id").val();
      var time_in = $("#modal-secondary #time_in").val();
      var time_out = $("#modal-secondary #time_out").val();
        $.ajax({
          type: "POST",
          url: "./Schedule/edit",
          data: {id:id, time_in:time_in, time_out:time_out},
          dataType: "json",
          success: function (data) {
            console.log(data);
          if(data==true)
