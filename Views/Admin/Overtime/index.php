<div class="content-wrapper" style="min-height: 1200.88px;">
              ...
              <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4"><div class="row"><div class="col-sm-12 col-md-6"><div class="dataTables_length" id="example1_length"><label>Show <select name="example1_length" aria-controls="example1" class="custom-select custom-select-sm form-control form-control-sm"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label></div></div><div class="col-sm-12 col-md-6"><div id="example1_filter" class="dataTables_filter" style="float: right;"><label>Search:<input type="search" class="form-control form-control-sm" placeholder="" aria-controls="example1"></label></div></div></div><div class="row"><div class="col-sm-12"><table id="example1" class="table table-bordered table-striped dataTable dtr-inline collapsed" role="grid" aria-describedby="example1_info">
                ...
                $id = 1;
                    foreach($data["list"] as $arr)
                    {
                        ?>
                        <tr>
                            <td id="id_overtime"><?= $arr["id_overtime"]?></td>
                            <td id="name_overtime">
                            <?php
                            ...
                            <td id="hour"><?= $arr["hour"]?></td>
                            <td id="start"><?= $arr["date_start"]?></td>
                            <td id="finish"><?= $arr["date_finish"]?></td>
                            <td id="rate"><?= $arr["rate"]?></td>
  ...
  ?>
        ...
        <script>
    $("#add").click(function(){
      var id = $("#modal-add #id option:selected").val();
      var hour = $("#modal-add #hour").val();
      var start = $("#modal-add #start").val();
      var finish = $("#modal-add #finish").val();
      var rate = $("#modal-add #rate").val();
      $.ajax({
        type: "POST",
        url: "./overtime/add",
        data: {id:id,hour:hour, start:start, finish:finish, rate},
        dataType: "Json",
    ...
    $(".edit").click(function(){
      ...
      $("#modal-secondary #id").val($(this).parents("tr").find("#id_overtime").text());
      $("#modal-secondary .info").html( $(this).parents("tr").find("#name_overtime").text())
      $("#modal-secondary #hour").val($(this).parents("tr").find("#hour").text());
      $("#modal-secondary #start").val($(this).parents("tr").find("#start").text());
      $("#modal-secondary #finish").val($(this).parents("tr").find("#finish").text());
      $("#modal-secondary #rate").val($(this).parents("tr").find("#rate").text());
    })
    $("#edit").click(function(){
      var id = $("#modal-secondary #id").val();
      var hour = $("#modal-secondary #hour").val();
      var start = $("#modal-secondary #start").val();
      var finish = $("#modal-secondary #finish").val();
    ...
    $(".delete").click(function()
      {
        var id = $(this).parents("tr").find("#id_overtime").text();
        Swal.fire({
          title: 'Are you sure?',
